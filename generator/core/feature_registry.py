...
class FeatureRegistry:
    ...
    @classmethod
    def fix_missing_dependencies(cls, selected_features: List[Type['BaseFeature']]) -> List[Type['BaseFeature']]:
        ...
        def add_missing_dependencies(feature: Type['BaseFeature']):
            for dep_name in getattr(feature, "dependencies", []):
                ...
                if not has_match:
                    logger.info(f"Missing dependency '{dep_name}' for feature '{feature.feature_name}'")
                    dep_class = all_features.get(dep_name)
                    if dep_class is not None:
                        logger.info(f"Added dependency '{dep_name}' with default configuration")
