...
class DocumentationGenerator:
    ...
    def _get_field_type_description(self, model_class: BaseModel, field_name: str, field_info: dict) -> str:
    ...
    def _format_type_annotation(self, type_annotation) -> str:
        ...
        if hasattr(type_annotation, '__name__'):
            if type_annotation.__name__ in ['str', 'int', 'float', 'bool', 'dict', 'list']:
    ...
    def _generate_markdown_docs(self, model_class: BaseModel, title: str = None, level: int = 0, output_description = True) -> str:
                ...
                if origin is not None:
                    ...
                    for arg in args:
                        if inspect.isclass(arg) and issubclass(arg, BaseModel):
                            docs.append(self._generate_markdown_docs(arg, title=f"{field_name} ({arg.__name__})", level=level+1, output_description=False))
                elif inspect.isclass(field_type_obj) and issubclass(field_type_obj, BaseModel):
                    docs.append(self._generate_markdown_docs(field_type_obj, title=f"{field_name} ({field_type_obj.__name__})", level=level+1, output_description=False))
            except Exception:
                pass
