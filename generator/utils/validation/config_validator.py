...
class ConfigValidator(BaseValidator):
    ...
    def _validate_features(self):
        ...
        for feature_name, feature_config in self.validated_config.features.items():
            ...
            if feature_name not in available_features:
                line_number = (
                    self.raw_config.get_line_number(f"features.{feature_name}")
                    if self.raw_config
                    else None
                )

                similar_features = self._find_similar_feature_names(
                    feature_name, available_features.keys()
                )
                suggestion = (
                ...
                if similar_features:
                    suggestion = (
                        f"Did you mean: {', '.join(similar_features)}? " + suggestion
                    )
            ...
            try:
                ...
                validation_context = {"config_file_path": self.config_path}
                feature_instance.get_feature_config(
                    self.validated_config, validation_context
                )

            except ValidationError as e:
                for error in e.errors():
                    field_path = f"features.{feature_name}." + ".".join(
                        str(loc) for loc in error["loc"]
                    )
                    line_number = (
                        self.raw_config.get_line_number(field_path)
                        if self.raw_config
                        else None
                    )
    ...
    def _find_similar_feature_names(
        self, target: str, available: List[str]
    ) -> List[str]:
        ...
        for feature in available:
            ...
            # Check for common prefixes/suffixes
                ...
                target_lower.startswith(feature_lower[:3])
                or feature_lower.startswith(target_lower[:3])
            ) and len(target) > 3:
                similar.append(feature)
