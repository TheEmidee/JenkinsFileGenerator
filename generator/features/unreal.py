...
class UnrealBuildGraphPostTasksConfig(BaseModel):
    ...
    enabled: bool = Field(default=True, description="If true, runs the post tasks (recordIssues) after the buildgraph tasks.")
    archive_artifacts: Optional[bool] = Field(default=True, description="If true, archives the artifacts generated by the recordIssues tasks.")

class UnrealBuildGraphConfig(BaseModel):
    target: str = Field(description="The target to build with Build Graph.")
    node_name_filters: Optional[Dict[str, str]] = Field(default=None,description="Filters to apply to the node names. Keys are the buildgraph task names and values are the jenkins node filter (Ex: `'MyGame Editor Win64 Test=BootTest': '!NoGPU'` will not select machine without a GPU to run the BootTest task).")
    properties: Optional[Dict[str,str]] = Field(default=None, description="Properties to pass to build graph. These are passed as -set:PropertyName=PropertyValue arguments to UAT.")
    pre_tasks: Optional[List[str]] = Field(default=None, description="List of tasks to run before the buildgraph tasks. These are run as shell commands. (Ex: `'pwsh script: \"Scripts/Project/CI/CI_NetUse.ps1\"'`)")
    post_tasks: Optional[UnrealBuildGraphPostTasksConfig] = Field(default_factory=UnrealBuildGraphPostTasksConfig(), description="Options to record issues on the logs generated by the buildgraph tasks")

class UnrealCleanupConfig(BaseModel):
    enabled: Optional[bool] = Field(default=True, description="If true, runs the cleanup tasks after the buildgraph.")
    additional_node_name : Optional[str] = Field(default=None, description="Additional jenkins node tags to use if you want the cleanup tasks to be executed on specific nodes.")
