...
```
  * **owner**: The GitHub owner of the repository. (  (Required) Type: `str` )

  * **repository**: The GitHub repository name. (  (Required) Type: `str` )

  * **credentials_id**: The jenkins credentials id that is associated with the GITHUB_TOKEN. (  (Required) Type: `str` )
  ...
  * **pull_requests**: Configuration for the pull requests (  Type: `Optional[features.github.GitHubPullRequestConfig]` Default: `None` )
    * **filter**: Filter configuration for pull requests, e.g., tokens to ignore. (  Type: `Optional[features.github.GitHubPullRequestFilterConfig]` Default: `None` )
      ...
      * **message**: Custom message to display when a pull request is ignored due to the filter. (  (Required) Type: `str` )
